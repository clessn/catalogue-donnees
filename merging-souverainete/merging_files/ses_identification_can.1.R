...
#### 2. clean variable
clean_pco <- NA
clean_pco[raw_pco == "Very strong" | raw_pco == "Somewhat strong"] <- 1
clean_pco[raw_pco != "Very strong" & raw_pco != "" & raw_pco != "Somewhat strong" & raw_pco != "Don't know"] <- 0
...
table(sondr::extract_elements_with_prefix(output_idcanqc, "pco"))
# Output ------------------------------------------------------------------
